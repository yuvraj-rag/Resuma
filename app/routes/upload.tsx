...
const upload = () => {
  ...
  const handleAnalyze = async ({companyName,jobTitle,jobDescription,file} : {companyName: string,jobTitle: string,jobDescription: string,file: File}) => {
    ...
    if(!imageFile.file) return setStatusText("failed to convert");

    setStatusText('Uploading Images...');
    const uploadedImage = await fs.upload([imageFile.file]);
    if(!uploadedImage) return setStatusText("failed to upload images");
    ...
    await kv.set(`resume:${uuid}`,JSON.stringify(data));
    setStatusText('Analysis Complete, Preparing Results')
    console.log(data);

    // fs.delete(uploadedFile.path);
    // fs.delete(uploadedImage.path);
        ...
        {!isProcessing && (
          <form id="upload-form" onSubmit={handleSubmit} className='flex flex-col gap-4 mt-8'>
            <div className='form-div'>
              <label htmlFor="commpany-name">Company Name</label>
